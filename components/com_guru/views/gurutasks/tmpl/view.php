...
//$catid = JRequest::getVar("catid", "0");
    ...
    <?php
		...
		$stop_next = 0;
		if(isset($_SESSION["stop_next"])){
			$stop_next = intval($_SESSION["stop_next"]);
			unset($_SESSION["stop_next"]);
		}
    ...
    <?php
		...
		$sql = "SELECT `score_quiz` FROM #__guru_quiz_question_taken_v3 WHERE user_id=".intval($user_id)." and quiz_id=".intval($result)." and pid=".$step->pid." ORDER BY id DESC LIMIT 0,1";
		$db->setQuery($sql);
		$db->query();
            ...
            <div class="uk-width-7-10 uk-text-right">
                ...
                if(@$step->prevs != 0 && $step->prevs != "-1"){
                    ...
                    if((($user->id <= 0) && (isset($step->prevaccess) && ($step->prevaccess != 2))) || ($user->id > 0 && !in_array($user->id, $author) && accessToLesson($step->prevs, $course_id) === FALSE && $step->prevaccess != 2)){
                        if($target == "0"){
                    ?>
                           <a href="#" onclick="openMyModal(400, 400, '<?php echo JRoute::_("index.php?option=com_guru&view=guruEditplans&course_id=".intval($catid)."&tmpl=component"); ?>');">
               ...
               if(isset($step->nexts) && ($step->nexts != 0)){
                ...
                ?>
                        <a href="#" onclick="openMyModal(400, 400, '<?php echo JRoute::_("index.php?option=com_guru&view=guruEditplans&course_id=".intval($catid)."&tmpl=component"); ?>')"><img style="border:none;" src="<?php echo JUri::base()."components/com_guru/images/next.png"; ?>" alt="<?php echo JText::_("GURU_NEXT_LESSON"); ?>" title="<?php echo JText::_("GURU_NEXT_LESSON"); ?>"/>
                        </a>
                <?php
                    ...
                    ?>
                                <a href="#" onclick="openMyModal(400, 400, '<?php echo JRoute::_("index.php?option=com_guru&view=guruTasks&catid=".intval($catid)."&task=view&module=".$next_module."&cid=".$cid."&tmpl=component"); ?>');">
                                    <img  style="border:none;" src="<?php echo JUri::base()."components/com_guru/images/next.png"; ?>" alt="<?php echo JText::_("GURU_NEXT_LESSON"); ?>" title="<?php echo JText::_("GURU_NEXT_LESSON"); ?>"/>
                                </a>
                    <?php
                            }
                                            	...
                                            	<div id="the_quiz"><!-- start quizz/final exam content -->
                                                	<form method="post" action="" name="quizz_exam" id="quizz_exam">
                                                    	<?php
															...
															if($result == 1){
																$sql = "SELECT show_countdown, max_score, nb_quiz_select_up  FROM #__guru_quiz WHERE id=".$id;
																$database->setQuery($sql);
																	...
																	if($callback == 0 || $callback == NULL){
																		if($result->show_countdown == 0){
																			if(trim(@$step->layout_media[0]) != ""){
																				...
																				$m1 = @$_COOKIE[intval($id)."-m1"];
																				$m2 = @$_COOKIE[intval($id)."-m2"];
																		...
																		if((trim($step->layout_media["0"]) != "") && $time_quiz_taken > 1 && $time_quiz_taken < 11){
