...
if ($status=='-1') 
	...
	function getCount($module, $lesson){
		$db = JFactory::getDBO();
		$sql = "select count(*) from #__guru_mediarel where `type`='scr_m' and `type_id`=".intval($lesson)." and `layout`='12'";
		$db->setQuery($sql);
		$db->query();
		$count = $db->loadColumn();
		$count = @$count["0"];
	...
	function getLessonDetails($program, $author, $lesson, $diff_date, $diff_start, $step_less, $start_date, $config, $lesson_details_for_quiz, $poz){
		$user = JFactory::getUser();
		$db = JFactory::getDbo();
		
		// check if logged user is the author of this course, he can open all course lessons
		if(intval($user->id) != 0 && strpos(" ".$program->author, "|".intval($user->id)."|") !== FALSE){
		...
		if(intval($user->id) != 0){ // logged user
			...
			if($program->course_type == 1 && intval($program->lesson_release) != 0){ // Sequential course, bot not all at once
				if($step_less != "" && $start_date != ""){
					...
					if($diff_start > 0 && $diff_date <= 0){
						$date_to_display = date($config->datetype, $date_to_display);
						$available_div = '<div class="available_lesson replace_class available pull-left">'.$date_to_display.'</div>';
						$lesson["can_open_lesson"] = "0";
						...
						if($program->lessons_show == 1){
							$available_div = '<div class="available_lesson replace_class available pull-left">'.JText::_("GURU_AVAILABLE").'</div>';
							$lesson["available_div"] = $available_div;
						}
					}
				}
