...
class guruModelguruauthor extends JModelList {
	...
	function getAuthorMyCourses(){
		...
		$sql = "SELECT * FROM #__guru_program WHERE (`author` like '%|".intval($user->id)."|%' OR `author`='".intval($user->id)."') ".$and;
...
<embed width="'.$vwidth.'" height="'.$vheight.'" type="video/x-msvideo" src="'.JURI::root().$configs->videoin."/".$the_media->local.'" name="plugin">
	...
	function addquestion ($qtext,$quizid,$question_type,$media_ids,$points,$true_false_ch, $question_id, $from_save_or_not,$ans_content) {
		...
		if($question_type == 'true_false'){
			$sql = "INSERT INTO `#__guru_question_answers`(`id`, `answer_content_text`, `media_ids`, `correct_answer`, `question_id`) VALUES ('', '".addslashes(JText::_("GURU_QUESTION_OPTION_TRUE"))."', '' , '".addslashes($true_false_ch)."',".intval($id_question["0"]).");";
			$db->setQuery($sql);
			if (!$db->query() ){
				$this->setError($db->getErrorMsg());
				return false;
			...
			$sql = "INSERT INTO `#__guru_question_answers`(`id`, `answer_content_text`, `media_ids`, `correct_answer`, `question_id`) VALUES ('', '".addslashes(JText::_("GURU_QUESTION_OPTION_FALSE"))."', '' , '".addslashes($true_false_ch)."', ".intval($id_question["0"]).");";
			$db->setQuery($sql);
			if (!$db->query() ){
				$this->setError($db->getErrorMsg());
				return false;
		...
		if($question_type == 'single'){
			$correct_ans = JRequest::getVar("correct_ans", array(), "post", "array");
			$ans_media_ids = JRequest::getVar("ans_media_ids", array(), "post", "array");
	...
	function updatequestion ($qtext,$quizid,$question_type,$media_ids,$points,$true_false_ch, $question_id, $from_save_or_not,$ans_content) {
		...
		// start true/false question
		if($question_type == 'true_false'){
			$sql = "UPDATE `#__guru_question_answers` set `correct_answer`= '".addslashes($true_false_ch)."' where `answer_content_text`='".addslashes(JText::_("GURU_QUESTION_OPTION_TRUE"))."' and `question_id`=".intval($question_id);
			$db->setQuery($sql);
			if (!$db->query() ){
				$this->setError($db->getErrorMsg());
				return false;
			...
			$sql = "UPDATE `#__guru_question_answers` set `correct_answer`= '".addslashes($true_false_ch)."' where `answer_content_text`='".addslashes(JText::_("GURU_QUESTION_OPTION_FALSE"))."' and `question_id`=".intval($question_id);
			$db->setQuery($sql);
			if (!$db->query() ){
				$this->setError($db->getErrorMsg());
				return false;
		...
		if($question_type == 'single'){
			...
			if(isset($ans_content) && is_array($ans_content) && count($ans_content) > 0){
				foreach($ans_content as $key=>$value){
					if(trim($value["0"]) == ""){
						if(!isset($ans_media_ids[$key])){
							$sql = "UPDATE `#__guru_question_answers` set `media_ids`='' where `id`=".intval($key);
							$db->setQuery($sql);
							$db->query();
							continue;
						}
				...
				foreach ($question_array as $question)
					{
						...
						$sql = "INSERT INTO `#__guru_questions_v3` 
															( 
																...
																'".$new_quiz_id."', 
																'".mysql_escape_string($the_question_object->type)."', 
																'".mysql_escape_string($the_question_object->question_content)."' , 
																'".mysql_escape_string($the_question_object->media_ids)."', 
																'".mysql_escape_string($the_question_object->points)."',
																'".mysql_escape_string($the_question_object->published)."',
																'".mysql_escape_string($the_question_object->question_order)."'
															)";
						$db->setQuery($sql);
						if (!$db->query() ){
							$this->setError($db->getErrorMsg());
	...
	function sendEmailForAskApprove($course_id){
		...
		if(isset($authors) && count($authors) > 0){
			foreach($authors as $key=>$author){
				...
				$sql = "select u.name as username from #__users u where u.`id`=".intval($author);
				$db->setQuery($sql);
				$db->query();
				$result = $db->loadAssocList();
				...
				for($i=0; $i< count($email); $i++){
					...
					$sql = "insert into #__guru_logs (`userid`, `emailname`, `emailid`, `to`, `subject`, `body`, `buy_date`, `send_date`, `buy_type`) values ('".intval($author)."', 'email-to-ask-approved', '0', '".$email[$i]."', '".addslashes(trim($subject))."', '".addslashes(trim($body))."', '', '".date("Y-m-d H:i:s")."', '')";
					$db->setQuery($sql);
					$db->query();
				}
			}
	...
	function storequizdetails(){
		...
		$pid = JRequest::getVar("pid", "0");
		$grade = JRequest::getVar("grade", array(), "post", "array");
		$feedback = JRequest::getVar("feedback", array(), "post", "array");
		$feedback_quiz_results = JRequest::getVar("feedback_quiz_results", array(), "post", "array");
		$total_points = 0;
		...
		$sql = "insert into #__guru_logs (`userid`, `emailname`, `emailid`, `to`, `subject`, `body`, `buy_date`, `send_date`, `buy_type`) values ('".intval($user_id)."', 'my-quiz-marcked', '0', '".$student_email."', '".addslashes(trim($subject))."', '".addslashes(trim($body))."', '', '".date("Y-m-d H:i:s")."', '')";
		$db->setQuery($sql);
		$db->query();
		...
		if(isset($feedback_quiz_results) && count($feedback_quiz_results) > 0){
			foreach($feedback_quiz_results as $question_id=>$text){
				...
				if($feedback_quiz_results != $text){
					...
					$sql = "insert into #__guru_logs (`userid`, `emailname`, `emailid`, `to`, `subject`, `body`, `buy_date`, `send_date`, `buy_type`) values ('".intval($user_id)."', 'change-feedback-quiz', '0', '".$student_email."', '".addslashes(trim($subject))."', '".addslashes(trim($body))."', '', '".date("Y-m-d H:i:s")."', '')";
					$db->setQuery($sql);
					$db->query();
				}
			}
	...
	function getForMark(){
		...
		$items = array();
		$filter_search = JRequest::getVar("filter_search", "");
		$filter_quiz = JRequest::getVar("filter_quiz", "0");
		$filter_quiz_type = JRequest::getVar("filter_quiz_type", "");
		...
		$course_id = JRequest::getVar("id", "0");
