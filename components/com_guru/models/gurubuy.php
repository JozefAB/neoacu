					...
					for($i=0; $i< count($email); $i++){
						...
						if(isset($idss) && count($idss) > 0){
							foreach($idss as $key=>$id){
								$sql = "insert into #__guru_logs (`userid`, `productid`, `emailname`, `emailid`, `to`, `subject`, `body`, `buy_date`, `send_date`, `buy_type`) values ('".intval($user_id)."', '".intval($id)."', 'buy-offline', '0', '".$email[$i]."', '".addslashes(trim($subject_procesed))."', '".addslashes(trim($body_procesed))."', '".date("Y-m-d H:i:s")."', '".date("Y-m-d H:i:s")."', '".trim($buy_type)."')";
								$db->setQuery($sql);
								$db->query();
							}
						}
