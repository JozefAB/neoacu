...
class guruModelguruPcateg extends JModelList {
	...
	function getprograms(){
		...
		$and = "";
		$guru_search = JRequest::getVar("guru_search", "");
		if(trim($guru_search) != ""){
			$and .= " AND `name` like '%".addslashes(trim($guru_search))."%'";
		}
		
		$query  = "SELECT * FROM `#__guru_program` WHERE published='1' AND status='1' AND startpublish <='".$date."' AND (endpublish >='".$date."' OR endpublish = '0000-00-00 00:00:00' ) AND catid=".intval($id)." ".$and." order by `ordering` asc";
		$this->_promos = $this->_getList($query);
