...
<embed width="'.$media->width.'" height="'.$media->height.'" name="plugin" src="'.JURI::root().$configs->videoin."/".$media->local.'" type="video/x-msvideo">
	...
	function saveLessonViewed($step_id,$pid){
		$db = JFactory::getDBO();
		$user = JFactory::getUser();
		$id = $user->id;
	...
	function getResultQuizzes($quiz_id, $course_id, $user_id, $number_of_questions){
		...
		if(isset($result_quiz->nb_quiz_select_up) && $result_quiz->nb_quiz_select_up !=0 && $result_quiz->show_nb_quiz_select_up ==0){
			$order_by = " ORDER BY RAND() LIMIT  ".$result_quiz->nb_quiz_select_up."";
		}
		else{
		...
		if($result_quiz->is_final == 1){
			...
			$query  = "SELECT * FROM #__guru_questions_v3 WHERE qid = ".intval($quiz_id)." and published=1".$order_by;
		}		
		$db->setQuery($query);
		$quiz_questions = $db->loadObjectList();
		...
		for($pag = 1; $pag <= $nr_pages; $pag++){
				...
				elseif($quiz_questions[$i]->type == "single"){
					if(isset($question_answers) && count($question_answers) > 0){
						...
						foreach($question_answers as $question_answer){
							...
							$option_value = '<input type="radio" '.$checked.' id="ans'.$question_answer->id.'" name="answers_single['.intval($quiz_questions[$i]->id).']" value="'.$question_answer->id.'"/><span class="lbl"></span>&nbsp;<span class="'.$correct_class.'">'.$question_answer->answer_content_text.'</span><br/>'.implode("<br/><br/>",$result_media_answers)."<br/>";
				...
				elseif($quiz_questions[$i]->type == "multiple"){
					if(isset($question_answers) && count($question_answers) > 0){
						...
						foreach($question_answers as $question_answer){
							...
							$option_value = '<input type="checkbox" '.$checked.' name="multiple_ans['.intval($quiz_questions[$i]->id).'][]" value="'.$question_answer->id.'"/>&nbsp;<span class="'.$correct_class.'">'.$question_answer->answer_content_text.'</span><br/>'.implode("",$result_media_answers)."<br/>";
			...
			if($pag == $nr_pages){
				$catid_req = intval(JRequest::getVar("catid",""));
				$module_req = intval(JRequest::getVar("module",""));
				$cid_req = intval(JRequest::getVar("cid",""));
				$quiz_form_content .='
		...
		if($result_quiz->time_quiz_taken >= 0){
			if($score >= $result_quiz->max_score){
				...
				$quiz_form_header .= '<span>'.$lang_quizpassed.'<span style="color:#669900;">'.$score.$percent.'</span>'.','." ".$min_to_pass." ".'<span style="color:#669900;">'.$result_quiz->max_score.$percent.'</span></span>';
				...
				if(!$exist_essay){
					...
					$_SESSION["stop_next"] = intval($student_failed);
					...
					if(isset($result_quiz->fail_message) && trim($result_quiz->fail_message) != ""){
						$quiz_form_header .= '<div class="guru-quiz-timer">'.trim($result_quiz->fail_message).'</div>';
					}
					
					$quiz_form_header .='<div class ="guru-quiz-timer">';
					$quiz_form_header .='<span>'.$lang_quiz.'<span style="color:#669900;">'.$score.$percent.'</span>'.','." "." ".$min_to_pass." ".'<span style="color:#669900;">'.$result_quiz->max_score.$percent.'</span></span>';
				}
				else{
	...
	function store_quiz_results(){
		...
		$user = JFactory::getUser();
		$user_id = $user->id;
		$quiz_id = JRequest::getVar("quize_id", 0);
		$pid = JRequest::getVar("pid", 0);
		$question_ids = array();
		$answers_single = JRequest::getVar("answers_single", array(), "post", "array");
		$truefs_ans = JRequest::getVar("truefs_ans", array(), "post", "array");
		$multiple_ans = JRequest::getVar("multiple_ans", array(), "post", "array");
		$essay_ans = JRequest::getVar("essay", array(), "post", "array");
		$all_question_from_page = JRequest::getVar("all_questions_ids", "");
		$config = JFactory::getConfig();
		$offset = JFactory::getApplication()->getCfg('offset');
		$jnow = JFactory::getDate('now', $offset);
		$date = $jnow;
		...
		if(isset($answers_single) && count($answers_single) > 0){
			foreach($answers_single as $question_id=>$value_id){
				$question_ids[] = $question_id;
				$sql = "INSERT INTO `#__guru_quiz_taken_v3`(`user_id`, `quiz_id`, `question_id`, `answers_given`, `pid`, `id_question_taken`) VALUES ('".intval($user_id)."', '".intval($quiz_id)."', '".intval($question_id)."', '".intval($value_id)."', '".intval($pid)."', '".$max_id."')";
				$db->setQuery($sql);
				$db->query();
			}
		}
		...
		if(isset($multiple_ans) && count($multiple_ans) > 0){
			foreach($multiple_ans as $question_id=>$values){
				...
				$sql = "INSERT INTO `#__guru_quiz_taken_v3`(`user_id`, `quiz_id`, `question_id`, `answers_given`, `pid`, `id_question_taken`) VALUES ('".intval($user_id)."', '".intval($quiz_id)."', '".intval($question_id)."', '".$values_string."', '".intval($pid)."', '".$max_id."')";
				$db->setQuery($sql);
				$db->query();
			}
		}
		...
		if(isset($essay_ans) && count($essay_ans)){
			foreach($essay_ans as $question_id=>$values){
				$values_string = "";
				$question_ids[] = $question_id;
				$sql = "INSERT INTO `#__guru_quiz_taken_v3`(`user_id`, `quiz_id`, `question_id`, `answers_given`, `pid`, `id_question_taken`) VALUES ('".intval($user_id)."', '".intval($quiz_id)."', '".intval($question_id)."', '".$values."', '".intval($pid)."', '".$max_id."')";
				$db->setQuery($sql);
				$db->query();
			}
		...
		foreach($result_diff as $key=>$value_id){
			$sql = "INSERT INTO `#__guru_quiz_taken_v3`(`user_id`, `quiz_id`, `question_id`, `answers_given`, `pid`, `id_question_taken`) VALUES ('".intval($user_id)."', '".intval($quiz_id)."', '".intval($value_id)."', '', '".intval($pid)."', '".$max_id."')";
			$db->setQuery($sql);
			$db->query();
		}
	...
	function sendEmailToTeacher(){
		...
		$app = JFactory::getApplication();
		$site_name = $app->getCfg('sitename');
		$quize_name = JRequest::getVar("quize_name", "");
		...
		if(isset($authors) && count($authors) > 0){
			foreach($authors as $key=>$author){
				...
				$sql = "insert into #__guru_logs (`userid`, `emailname`, `emailid`, `to`, `subject`, `body`, `buy_date`, `send_date`, `buy_type`) values ('".intval($author)."', 'teacher-mark-essay', '0', '".trim($teacher_email)."', '".addslashes(trim($subject))."', '".addslashes(trim($body))."', '', '".date("Y-m-d H:i:s")."', '')";
				$db->setQuery($sql);
				$db->query();
			}
		}
