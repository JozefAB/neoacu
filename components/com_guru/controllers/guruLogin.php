...
class guruControllerguruLogin extends guruController {
	...
	function sendJoomlaEmail($data){
		...
		$sql = "insert into #__guru_logs (`userid`, `emailname`, `emailid`, `to`, `subject`, `body`, `buy_date`, `send_date`, `buy_type`) values ('".intval($data["id"])."', 'user-registration', '0', '".trim($data['email'])."', '".addslashes(trim($emailSubject))."', '".addslashes(trim($emailBody))."', '', '".$data["registerDate"]."', '')";
		$db->setQuery($sql);
		$db->query();
		...
		if (($params->get('useractivation') < 2) && ($params->get('mail_to_admin') == 1))
		{
			...
			foreach ($rows as $row)
			{
				...
				$sql = "insert into #__guru_logs (`userid`, `emailname`, `emailid`, `to`, `subject`, `body`, `buy_date`, `send_date`, `buy_type`) values ('".intval($data["id"])."', 'user-registration', '0', '".trim($row->email)."', '".addslashes(trim($emailSubject))."', '".addslashes(trim($emailBodyAdmin))."', '', '".$data["registerDate"]."', '')";
				$db->setQuery($sql);
				$db->query();
