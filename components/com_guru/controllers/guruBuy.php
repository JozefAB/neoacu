...
class guruControllerguruBuy extends guruController {
	...
	function checkout(){
		...
		foreach($items as $key=>$value){
			$plan_selected = JRequest::getVar("plan_selected", array(), "post", "array");
			...
			/*if(isset($course_plans) && count($course_plans) > 0){
				...
				foreach($course_plans as $key_plan=>$value_plan){
					$code = md5('guru-poz-'.$key_plan);
					if($plan_selected[$value["course_id"]] == $code){
						if($value_plan["price"] != $plan_id[$value["course_id"]]){
							$find_error = true;
						}
						$ok = true;
					}
		...
		if($total_prices == "0" || $total_prices == "0.00" || (isset($discount_value) && trim($discount_value) != "" && ($total_prices - $discount_value) == 0 && $promo_details->typediscount == '0') && $promo_details->typediscount == '0'){
			$model = $this->getModel("guruBuy");
