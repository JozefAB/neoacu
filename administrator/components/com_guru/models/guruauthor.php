...
class guruAdminModelguruauthor extends JModelLegacy{
	...
	function save(){
		...
		/*if(isset($data['gid'])){
			if(intval($data['userid']) != 0){
				$sql = "delete from `#__user_usergroup_map` where `user_id`='".$data['userid']."'";
				$db->setQuery($sql);
				if($db->query()){
					foreach($data['gid'] as $key_gid=>$value_gid){
						$sql = "insert into `#__user_usergroup_map` (`user_id`, `group_id`) values('".$data['userid']."', '".$value_gid."')";
						$db->setQuery($sql);
						$db->query();
					}
				}
